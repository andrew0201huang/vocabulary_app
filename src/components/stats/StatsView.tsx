    ...
    <div className="w-full max-w-5xl mx-auto flex flex-col gap-6 px-4 py-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="px-4 py-2 rounded-xl text-white text-xs font-bold shadow-md transition-all active:scale-95 hover:opacity-90" style={{ backgroundColor: 'var(--accent)' }}
        >
          開始新回合
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-4 rounded-2xl border bg-white border-slate-200 shadow-sm flex flex-col">
          <span className="text-xs text-slate-400 font-medium">總測驗次數</span>
          <span className="text-2xl sm:text-3xl font-extrabold text-slate-900 font-mono mt-1">
            {totalPracticed}
          </span>
          <span className="text-[11px] text-slate-400 mt-0.5">累計作答單字</span>
        </div>
        ...
        <div className="p-4 rounded-2xl border bg-white border-slate-200 shadow-sm flex flex-col">
          ...
          <span className="text-[11px] text-slate-400 mt-0.5">{totalCorrect} 次正確</span>
        </div>
        ...
        <div className="p-4 rounded-2xl border bg-white border-slate-200 shadow-sm flex flex-col">
          ...
          <span className="text-[11px] text-slate-400 mt-0.5">全庫單字平均</span>
        </div>
        ...
        <div className="p-4 rounded-2xl border bg-white border-slate-200 shadow-sm flex flex-col">
          ...
          <span className="text-[11px] text-slate-400 mt-0.5">最佳直覺拼寫</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Speed Distribution */}
        <div className="p-5 rounded-3xl border bg-white border-slate-200 shadow-sm flex flex-col gap-4 shadow-md">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-slate-900 flex items-center gap-2">
              <Zap className="w-4 h-4 text-amber-400" />
              <span>反應速度分佈 (Speed Distribution)</span>
            </h2>
            <span className="text-xs text-slate-400">已測驗 {totalEvaluated} 字</span>
          </div>
          ...
          <div className="flex flex-col gap-3">
            <div>
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="font-mono text-slate-700">{fastCount} 字 ({totalEvaluated ? Math.round((fastCount / totalEvaluated) * 100) : 0}%)</span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="font-mono text-slate-700">{goodCount} 字 ({totalEvaluated ? Math.round((goodCount / totalEvaluated) * 100) : 0}%)</span>
              </div>
              <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                <div
            ...
            <div>
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="font-mono text-slate-700">{slowCount} 字 ({totalEvaluated ? Math.round((slowCount / totalEvaluated) * 100) : 0}%)</span>
              </div>
              <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                <div
            ...
            <div>
              <div className="flex justify-between text-xs mb-1">
                <span className="text-rose-400 font-semibold">生疏遲疑 (&gt; 6.0s / 錯誤)</span>
                <span className="font-mono text-slate-700">{overtimeCount} 字 ({totalEvaluated ? Math.round((overtimeCount / totalEvaluated) * 100) : 0}%)</span>
              </div>
              <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden">
                <div
        ...
        <div className="p-5 rounded-3xl border bg-white border-slate-200 shadow-sm flex flex-col gap-4 shadow-md">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400">總庫存 {words.length} 字</span>
          </div>
          ...
          <div className="p-2.5 rounded-xl bg-slate-50 text-xs text-slate-400 flex items-center justify-between">
            <span>尚未測驗之新單字</span>
            <span className="font-mono font-bold text-slate-900">{newCount} 個</span>
          </div>
        </div>
      </div>

      {/* Recent Round History */}
      <div className="p-5 rounded-3xl border bg-white border-slate-200 shadow-sm flex flex-col gap-3 shadow-md">
        <h2 className="text-sm font-bold text-slate-900 flex items-center gap-2">
        ...
        {roundHistory.length === 0 ? (
          ...
          <div className="flex flex-col gap-2 max-h-64 overflow-y-auto custom-scrollbar">
            {roundHistory.map((round) => (
              <div
                key={round.id}
                className="flex items-center justify-between p-3 rounded-xl border bg-slate-50 border-slate-200 text-xs"
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-indigo-50 border border-indigo-200 flex items-center justify-center font-mono font-bold text-indigo-300">
                    {round.accuracyRate}%
                  </div>
                  <div>
