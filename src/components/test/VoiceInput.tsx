...
export const VoiceInput: React.FC<VoiceInputProps> = ({
  ...
  return (
    <div className="w-full flex flex-col items-center gap-4">
      ...
      {/* Mic Action Card */}
      <div className="w-full max-w-md p-5 rounded-2xl bg-white border border-slate-200 flex flex-col items-center gap-3 text-center shadow-lg">
        {/* Mic Button */}
        <button
          type="button"
          ...
          className={`w-16 h-16 rounded-full flex items-center justify-center transition-all transform active:scale-95 shadow-xl ${
            isListening
              ? 'bg-rose-500 text-slate-900 ring-4 ring-rose-500/30 scale-105 animate-pulse'
        ...
        <div className="flex items-center gap-2 w-full pt-1">
          <button type="button" onClick={handleDeleteLast}
            ...
            className="flex-1 py-2 px-3 rounded-lg bg-slate-100 hover:bg-slate-200 disabled:opacity-30 text-xs font-semibold text-slate-700 flex items-center justify-center gap-1.5 transition-colors">
          ...
          <button type="button" onClick={handleClear}
            disabled={!spelledBuffer || disabled}
            className="py-2 px-3 rounded-lg bg-slate-100 hover:bg-slate-200 disabled:opacity-30 text-xs font-semibold text-slate-600 hover:text-rose-400 transition-colors">
            清空
          </button>

          <button type="button" onClick={handleSubmit}
            disabled={!spelledBuffer || disabled}
            className="flex-1 py-2 px-3 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-30 text-xs font-bold text-slate-900 flex items-center justify-center gap-1.5 transition-colors shadow-md">
